...
PSNR(frame_no)=psnr1(i,j)
SSIM(frame_no)=ssim(i,j)
...
K_rgb = ind2rgb(K1,'colormap');
K_rgb=im2uint8(K_rgb);
mov(frame_no).cdata = K_rgb;
    

PSNR_ORIGNAL_WATERMARKED(frame_no)=psnr1(Original_image,K1)
SSIM_ORIGNAL_WATERMARKED(frame_no)=ssim(Original_image,K1)
...
SIM=sum(watermark.*watermark_n)/sqrt(sum(watermark.*watermark_n)); 
SIM_ORIGNAL_WATERMARKED(frame_no)=SIM
